...
enum WeatherLocationSource {
    ...
    /// Cache key for location-based deduplication
    var cacheKey: String {
        switch self {
        case .coordinates(let lat, let lon):
            let rlat = String(format: "%.2f", lat)
            let rlon = String(format: "%.2f", lon)
            return "coords:\(rlat),\(rlon)"
        case .city(let name):
            return "city:\(name.lowercased().trimmingCharacters(in: .whitespaces))"
        }
    }
}
...
struct OpenWeather {
    ...
    static func fetch(location: WeatherLocationSource,
                      completion: @escaping (Result<OWeather, NetworkError>) -> Void) {
        ...
        fetchData(from: urlString) { result in
            ...
            case .success(let jsonString):
                ...
                do {
                    let weather = try newJSONDecoder().decode(OWeather.self, from: jsonData)
                    completion(.success(weather))
                } catch {
                    debugLog("=== OW: decode failed: \(error)")
