    ...
    var body: some View {
        ScrollViewReader { proxy in
            ...
            .onChange(of: entries.count) { newCount in
                // A shrinking playlist (switching from a large playlist,
                // scrolled deep, to a small one) leaves the scroll offset
                // beyond the new content — an empty viewport. Re-anchor
                // once layout settles.
    ...
    private func cachedRow(entry: PlaylistEntry, index: Int) -> some View {
        ...
        return HStack(spacing: 8) {
            ...
            ZStack(alignment: .leading) {
                ...
                // Play/pause overlay — visual states (battery-paused,
                // thermal/LPM-paused, user-paused, playing) mirroring
                // PlaylistStripView.
                if isCurrent {
