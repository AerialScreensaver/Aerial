...
struct OverlaysSettingsPanel: View {
    ...
    #if DEBUG
    @State private var alwaysShowVersion: Bool = OverlayConfigManager.shared.config.alwaysShowVersion
    #endif
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Toggle("Shift overlays clear of the Dock and menu bar", isOn: $dockOffsetEnabled)
                        .onChange(of: dockOffsetEnabled) { _, newValue in
                            var config = OverlayConfigManager.shared.config
                            config.dockOffsetEnabled = newValue
                            OverlayConfigManager.shared.setConfig(config)
                        }

                    Text("In wallpaper mode, moves overlays inward so they aren't covered by the Dock or menu bar. The screensaver is never offset.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                    ...
                    Toggle("Always show version (debug)", isOn: $alwaysShowVersion)
                        .onChange(of: alwaysShowVersion) { _, newValue in
                            var config = OverlayConfigManager.shared.config
                            config.alwaysShowVersion = newValue
                            OverlayConfigManager.shared.setConfig(config)
                        }

                    Text("Debug build only — keeps the version pinned at the bottom-right in both wallpaper and screensaver modes (never auto-hides). Stripped from Release builds.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                    #endif
