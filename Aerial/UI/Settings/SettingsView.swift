...
struct SettingsView: View {
    @State private var selectedPanel: SettingsPanel = .desktop
    @Environment(\.openWindow) private var openWindow

    init() {
        // Honor a deferred panel set by a caller right before
        // `SettingsWindowController.show(panel:via:)` (e.g. Home "More options…").
        _selectedPanel = State(initialValue: SettingsWindowController.pendingInitialPanel ?? .desktop)
        SettingsWindowController.pendingInitialPanel = nil
    }
    ...
    var body: some View {
        ...
        .tint(.aerial)
        // Already-open window: a caller asked to jump to a specific panel.
        .onReceive(NotificationCenter.default.publisher(for: SettingsWindowController.openPanelRequest)) { note in
            if let panel = note.object as? SettingsPanel { selectedPanel = panel }
        }
    }
}
