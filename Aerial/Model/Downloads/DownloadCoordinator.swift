...
class DownloadCoordinator {
    ...
    func enqueueAllVideos(forSource sourceName: String) {
        guard Cache.isAvailable else {
            debugLog("💽 DownloadCoordinator: enqueueAllVideos skipped — external cache image not attached")
            return
        }
    ...
    private func runScheduledCheck() {
        ...
        guard Cache.isAvailable else {
            debugLog("💽 DownloadCoordinator: scheduled check skipped — external cache image not attached")
            return
        }

        // Refresh manifests so any new Apple/community videos are discovered.
        // `reloadSources()` synchronously re-parses the cached manifests and
        // kicks the network refresh in the background. The fill/rotate
        // decision MUST wait for that synchronous rebuild: this used to be a
        // fire-and-forget main.async while we carried straight on here, and
        // on a build whose `refreshVideoList` empties `videos` before
        // re-appending, `Cache.outdatedVideos()` + `evaluateAndDownload()`
        // ran against an EMPTY catalog — "No outdated videos", cached=0
        // uncached=0, nothing evicted or queued, and `lastRotationRun`
        // stamped anyway (2026-08 4.0.14 log: 5/5 rotation cycles on a
        // 20 GB cache that never rotated). So: reload on main, then come
        // back to `workQueue` for phase 2.
        DispatchQueue.main.async { [weak self] in
            VideoList.instance.reloadSources()
            self?.workQueue.async { [weak self] in
                self?.runScheduledCheckPhase2()
            }
        }
    }

    /// Fill / rotate decision — runs on `workQueue` AFTER the catalog was
    /// rebuilt from the cached manifests (see `runScheduledCheck`).
    private func runScheduledCheckPhase2() {
        // Re-check: sleep could have started while we hopped queues.
        guard !SystemSleepState.shared.isAsleep else {
            debugLog("DownloadCoordinator: scheduled check skipped (system asleep / dark wake)")
            return
        }
        ...
        // A genuinely empty catalog (sources not parsed yet) must never burn
        // the cadence window — defer, don't stamp.
        guard !VideoList.instance.videos.isEmpty else {
            debugLog("DownloadCoordinator: catalog empty — deferring rotation (not stamping lastRotationRun)")
            return
        }

        debugLog("DownloadCoordinator: rotation cycle starting (cadence=\(PrefsCache.cachePeriodicity))")
        // Evicts all outdated videos (cadence-aware), sparing what's on screen.
        Cache.freeCache(protecting: WallpaperStatusMonitor.nowPlayingVideoIds())
    ...
    private func evaluateAndDownload() {
        ...
        // External-drive cache: never download into the bare mount point.
        guard Cache.isAvailable else {
            debugLog("💽 DownloadCoordinator: evaluate skipped — external cache image not attached")
            return
        }
        ...
        // Refresh playlists after evaluation. regenerateAll() refreshes
        // every scope with its own stored filter and skips active user
        // playlists by construction — the no-mode regenerate() here used
        // to clobber an active user playlist back to the global filter
        // on every hourly/wake evaluation.
        PlaylistManager.shared.regenerateAll()
    }
    ...
    private func downloadCritical(uncached: [AerialVideo]) {
        ...
        // Non-cacheable sources write to their own folder (default
        // Sources root, or `<cache location>/Expansions` beside the
        // cache), which `Cache.size()` never counts (shallow, cache folder
        // only) — applying the gate here would (incorrectly) block
    ...
    private func forceEvictOneForCritical() {
        // Same oldest-first order as every other evictor; additionally
        // skips hidden videos (they're the rotation's own first tier) and
        // whatever is on screen right now.
        let hidden = Set(PrefsVideos.hidden)
        let protected = WallpaperStatusMonitor.nowPlayingVideoIds()
        guard let video = Cache.evictionCandidates(olderThan: nil)
                .first(where: { !hidden.contains($0.id) && !protected.contains($0.id) }),
