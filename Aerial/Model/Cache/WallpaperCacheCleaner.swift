...
//
//  Gated on `Preferences.cleanWallpaperCache` (opt-in, in Storage
//  settings). macOS 26+ only; older versions don't have the bug.
//
...
final class WallpaperCacheCleaner {
    ...
    /// Called from `AppDelegate.continueStartup()` at app launch.
    /// Silently activates monitoring if the pref is on AND the user has
    /// already granted access in a prior session. Never prompts here —
    /// a surprise NSOpenPanel at launch is jarring.
    func bootstrap() {
    ...
    /// Called whenever `cleanWallpaperCache` might have just changed —
    /// the Storage settings toggle or an explicit "Grant access" button.
    /// If the pref is on and we lack a bookmark, this is where the
    /// NSOpenPanel fires.
    func reevaluate() {
        guard #available(macOS 26.0, *) else { return }
        let shouldRun = Preferences.cleanWallpaperCache
        if shouldRun {
            if bookmarkData != nil {
                startMonitoring()
            } else {
