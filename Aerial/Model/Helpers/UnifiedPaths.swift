...
struct UnifiedPaths {
    ...
    static func ensureBaseDirectory() -> Bool {
        ...
        if fileManager.fileExists(atPath: basePath) {
            ...
            if fileManager.fileExists(atPath: markerPath) {
                ...
                debugLog("🚀 UnifiedPaths: Pre-existing folder found without companion.json, renaming")
                ...
                do {
                    ...
                    if fileManager.fileExists(atPath: renamedPath) {
                        ...
                        debugLog("🚀 UnifiedPaths: Renamed to \(finalRenamedPath)")
                    } else {
                        try fileManager.moveItem(atPath: basePath, toPath: renamedPath)
                        debugLog("🚀 UnifiedPaths: Renamed to Aerial-user")
                    }

                    // Inform user about the rename
                    showRenameNotification(newPath: finalRenamedPath)
                } catch {
                    errorLog("🚀 UnifiedPaths: Failed to rename pre-existing folder: \(error.localizedDescription)")
        ...
        if !alreadyInitialized {
            debugLog("🚀 UnifiedPaths: Starting initialization")
            debugLog("🚀 UnifiedPaths: Creating base directory")
            do {
                ...
                errorLog("🚀 UnifiedPaths: Failed to create base directory: \(error.localizedDescription)")
                showPermissionError(details: "Could not create /Users/Shared/Aerial/ directory.\n\nError: \(error.localizedDescription)")
                return false
            }

            // Step 3: Create companion.json marker (empty file)
            debugLog("🚀 UnifiedPaths: Creating companion.json marker")
            do {
                try "".write(toFile: markerPath, atomically: true, encoding: .utf8)
            } catch {
                errorLog("🚀 UnifiedPaths: Failed to create companion.json: \(error.localizedDescription)")
        ...
        if !alreadyInitialized {
            debugLog("🚀 UnifiedPaths: Ensuring subdirectories exist")
        }
        ...
        do {
            ...
            errorLog("🚀 UnifiedPaths: Failed to create Logs directory: \(error.localizedDescription)")
        ...
        do {
            ...
            errorLog("🚀 UnifiedPaths: Failed to create My Videos directory: \(error.localizedDescription)")
            // Non-fatal, but log it
        }

        if alreadyInitialized {
            debugLog("🚀 UnifiedPaths: Initialized")
        } else {
            debugLog("🚀 UnifiedPaths: Initialization complete!")
        }
        return true
    }
    ...
    private static func showRenameNotification(newPath: String) {
        DispatchQueue.main.async {
            ...
            We found an existing /Users/Shared/Aerial/ folder that wasn't created by Aerial.
