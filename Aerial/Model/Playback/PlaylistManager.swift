...
class PlaylistManager {
    ...
    /// Last CONTENT signature signaled to the wallpaper extension, per
    /// scope ("shared" or screenUUID). Seeded from the loaded state at
    /// init so a Companion relaunch with unchanged playlists signals
    /// nothing — `persist()` used to bump every screen's
    /// playlistGeneration unconditionally, which force-advanced the
    /// extension's videos ~6× on every app start (and would have on
    /// every progress write).
    private var lastSignaledSignatures: [String: Int] = [:]
    ...
    private init() {
        ...
        // Baseline: what's on disk is what the extension already plays.
        // Seeded BEFORE the sidecar merge below — that merge only moves
        // currentIndex / playbackTimestamp (not part of the content
        // signature) and persists, and seeding after it compared against
        // an empty baseline, signaling playlistDidChange(all) to the
        // extension on every launch.
        lastSignaledSignatures = Self.signatures(of: state)

        // Incorporate extension progress sidecar if present
        incorporateExtensionProgress()
    }

    // MARK: - Content Signatures

    /// Signature of a playlist's CONTENT — filter, cycle mode, and the
    /// ordered entry list (order matters so reshuffles still signal).
    /// Progress fields (`currentIndex`, `playbackTimestamp`,
    /// `generatedAt`) are deliberately excluded: progress bookkeeping
    /// must not look like a playlist change to the extension.
    private static func contentSignature(of playlist: PersistedPlaylist) -> Int {
        var hasher = Hasher()
        hasher.combine(playlist.filterMode)
        hasher.combine(playlist.filterStrings)
        hasher.combine(playlist.cycleMode.rawValue)
        for entry in playlist.entries {
            hasher.combine(entry.videoId)
            hasher.combine(entry.playDuration)
        }
        return hasher.finalize()
    }

    private static func signatures(of state: PlaylistState) -> [String: Int] {
        var sigs: [String: Int] = [:]
        if let shared = state.sharedPlaylist {
            sigs["shared"] = contentSignature(of: shared)
        }
        for (uuid, playlist) in state.screenPlaylists {
            sigs[uuid] = contentSignature(of: playlist)
        }
        return sigs
    }
    ...
    /// prefs. An active user playlist (filterMode == -1) is an explicit
    /// user choice and is refreshed from its manifest instead of being
    /// replaced — the old fallthrough to global prefs was the "playlist
    /// deselected itself" bug (every hourly/wake download evaluation
    /// clobbered it). Fall back to global `PrefsVideos` only when no
    /// prior playlist exists at all.
    func regenerate(for screenUUID: String? = nil) {
        let existing: (filterMode: Int, filterStrings: [String])? = queue.sync {
            guard let playlist = resolvePlaylist(screenUUID) else { return nil }
            return (playlist.filterMode, playlist.filterStrings)
        }

        if existing?.filterMode == -1 {
            reloadActiveUserPlaylistIfNeeded(for: screenUUID)
            return
        }

        if let existing, let mode = NewShouldPlay(rawValue: existing.filterMode) {
            regenerate(for: screenUUID, mode: mode, filterStrings: existing.filterStrings)
        } else {
            regenerate(for: screenUUID,
                       mode: PrefsVideos.newShouldPlay,
                       filterStrings: PrefsVideos.newShouldPlayString)
    ...
    func regenerateAll() {
        // Defense-in-depth: a transiently empty video list (mid-reloadSources
        // rebuild) must never be persisted as empty playlists and signaled to
        // the extension — every scope would be clobbered to [] and paused
        // wallpapers would visibly cut. The reload's completion callback
        // re-triggers regeneration once the list is repopulated.
        guard !VideoList.instance.videos.isEmpty else {
            debugLog("📋 regenerateAll skipped — video list is empty (transient reload?)")
            return
        }
        queue.sync(flags: .barrier) {
            if mergeExtensionProgress() {
    ...
    /// Single entry point for the playback-mode selectors (popover strip,
    /// dashboard Order card). Switching to shuffle keeps the historical
    /// immediate-visible-reshuffle behavior; loop and repeat-one are pure
    /// mode sets that never touch entries or order.
    func setPlaybackMode(_ mode: PlaylistCycleMode) {
        Preferences.playlistMode = mode
        if mode == .shuffle {
            reshuffleAll()
        } else {
            setCycleModeAll(mode)
        }
    }

    /// Set `cycleMode` on every playlist (shared + per-screen, including
    /// active user playlists) without mutating entries, order, or position.
    /// The content signature hashes `cycleMode`, so `persist()` bumps the
    /// playlist generation and the extension picks the mode up.
    func setCycleModeAll(_ mode: PlaylistCycleMode) {
        queue.sync(flags: .barrier) {
            if var shared = state.sharedPlaylist, shared.cycleMode != mode {
                shared.cycleMode = mode
                state.sharedPlaylist = shared
            }
            for (uuid, var playlist) in state.screenPlaylists where playlist.cycleMode != mode {
                playlist.cycleMode = mode
    ...
    /// Index the user explicitly selected per scope (keyed `screenUUID ?? "broadcast"`,
    /// matching `applyExtensionNowPlaying`'s echo keys), awaiting the extension's status
    /// echo to confirm it. While a jump is pending, echoes reporting a DIFFERENT index are
    /// ignored — they're the stale pre-swap status the extension writes synchronously right
    /// after a jump, before its async asset swap lands. Cleared on confirmation or timeout.
    private var pendingJump: [String: (index: Int, at: Date)] = [:]
    private static let pendingJumpTimeout: TimeInterval = 10

    /// Last trusted now-playing echo per scope key ("broadcast" or screen
    /// UUID) from the extension's status channel. `mergeExtensionProgress`
    /// prefers this over the progress sidecar's index while it is fresh:
    /// pre-fix extensions wrote their playlist CURSOR (one entry ahead of
    /// the screen) to the sidecar, and merging that moved the Library
    /// highlight one video forward on every launch / regenerate.
    private var extensionNowPlaying: [String: (videoId: String, at: Date)] = [:]
    /// Same window WallpaperStatusMonitor uses for "extension alive".
    private static let extensionNowPlayingFreshness: TimeInterval = 90
    ...
    func setCurrentIndex(_ index: Int, for screenUUID: String? = nil) {
        queue.sync(flags: .barrier) {
            ...
            needsResume = true
            // A manual jump is authoritative until the extension's echo confirms it
            // (see `applyExtensionNowPlaying`), so the stale pre-swap echo can't revert
            // the optimistic highlight back to the previous item.
            pendingJump[screenUUID ?? "broadcast"] = (playlist.currentIndex, Date())
            setPlaylist(playlist, for: screenUUID)
            persist()
    ...
    /// Sync each scope's `currentIndex` to the video the wallpaper
    /// extension reports it is actually rendering, from the status
    /// channel's per-renderer videoId map (`WallpaperStatusState.nowPlayingId`).
    /// This is what keeps the dashboard/popover "now playing" + highlight
    /// matching reality: the extension owns the live position, Companion
    /// observes it. In-memory only — no resume-timestamp writes, no
    /// persist, and no extension re-signal (currentIndex isn't part of the
    /// content signature). Posts `playlistDidChangeNotification` so the UI
    /// re-queries.
    func applyExtensionNowPlaying(_ ids: [String: String]) {
        var changed = false
        queue.sync(flags: .barrier) {
            for (key, videoId) in ids {
                // "broadcast" is the extension's shared-renderer key
                // (cloned / spanned / mirrored); any other key is an
                // independent per-screen UUID.
                let screenUUID: String? = (key == "broadcast") ? nil : key
                guard var playlist = resolvePlaylist(screenUUID),
                      let idx = playlist.entries.firstIndex(where: { $0.videoId == videoId }) else { continue }
                // Honor a pending user jump: ignore the stale pre-swap echo until the
                // extension confirms the selected index (or the safety timeout lapses).
                if let pending = pendingJump[key] {
                    if idx == pending.index {
                        pendingJump[key] = nil                  // extension confirmed the jump
                    } else if Date().timeIntervalSince(pending.at) < Self.pendingJumpTimeout {
                        continue                                // stale pre-swap echo — ignore
                    } else {
                        pendingJump[key] = nil                  // timed out — extension diverged
                    }
                }
                extensionNowPlaying[key] = (videoId, Date())
                guard idx != playlist.currentIndex else { continue }
    ...
    /// Get the current index for a screen's playlist, clamped to the
    /// entries range (mirrors `currentEntry`'s defense — a transiently
    /// stale index from a larger previous playlist must never leak to
    /// the UI). Returns 0 if no playlist exists.
    func currentIndex(for screenUUID: String? = nil) -> Int {
        return queue.sync {
            guard let playlist = resolvePlaylist(screenUUID),
                  !playlist.entries.isEmpty else { return 0 }
            return min(max(0, playlist.currentIndex), playlist.entries.count - 1)
        }
    }

    // (Index helpers removed — global next/previous now go through
    // the wallpaper extension's filter-aware forward/backward scan.)
    ...
    private func persist() {
        ...
        // Signal the wallpaper extension ONLY for scopes whose CONTENT
        // changed. persist() runs constantly (index pops, playback
        // timestamps, regenerate waves at startup) — unconditional
        // signaling bumped playlistGeneration on every screen each time
        // and made the extension drop + advance its videos on every
        // Companion launch.
        let current = Self.signatures(of: state)
        guard current != lastSignaledSignatures else { return }

        let sharedChanged = current["shared"] != lastSignaledSignatures["shared"]
        let allScreenKeys = Set(current.keys).union(lastSignaledSignatures.keys).subtracting(["shared"])
        let changedScreens = allScreenKeys.filter { current[$0] != lastSignaledSignatures[$0] }
        lastSignaledSignatures = current

        if sharedChanged {
            // The shared playlist serves every display in cloned/spanned
            // mode — bump all screens (the extension's broadcast renderer
            // matches any screen's signal).
            WallpaperControl.shared.playlistDidChange(screenUUID: nil)
        } else {
            for uuid in changedScreens {
                WallpaperControl.shared.playlistDidChange(screenUUID: uuid)
            }
        }
    }
    ...
    private func mergeExtensionProgress() -> Bool {
        ...
        if let sharedProgress = progress.sharedProgress, var shared = state.sharedPlaylist {
            if sharedProgress.updatedAt > shared.generatedAt {
                shared.currentIndex = mergedIndex(sidecar: sharedProgress.currentIndex, playlist: shared, key: "broadcast")
        ...
        for (uuid, screenProgress) in progress.screenProgress {
            if var playlist = state.screenPlaylists[uuid] {
                if screenProgress.updatedAt > playlist.generatedAt {
                    playlist.currentIndex = mergedIndex(sidecar: screenProgress.currentIndex, playlist: playlist, key: uuid)
    ...
    /// The index to adopt from a sidecar merge: the sidecar's, unless the
    /// extension's status channel has told us (freshly) which video is
    /// on screen and that video is in the playlist — the echo wins. The
    /// sidecar of a pre-fix extension is its cursor, one entry ahead.
    private func mergedIndex(sidecar: Int, playlist: PersistedPlaylist, key: String) -> Int {
        let clamped = min(sidecar, max(playlist.entries.count - 1, 0))
        guard let echo = extensionNowPlaying[key],
              Date().timeIntervalSince(echo.at) < Self.extensionNowPlayingFreshness,
              let echoed = playlist.index(ofVideoId: echo.videoId, searchingBackFrom: clamped)
        else { return clamped }
        if echoed != clamped {
            debugLog("[Playlist] sidecar index \(clamped) ≠ extension now-playing \(echoed) (\(key.prefix(8))) — using \(echoed)")
        }
        return echoed
    }
