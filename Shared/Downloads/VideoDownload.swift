...
final class VideoDownload: NSObject, URLSessionDownloadDelegate {
    ...
    func urlSession(_ session: URLSession,
        ...
        // External-drive cache: a download that was already in flight when
        // the disk image got detached must not land in the bare mount point
        // (it would be hidden — and wasted — the moment the image re-attaches).
        guard Cache.isAvailable else {
            errorLog("💽 Discarding download of \(video.name) — external cache image not attached")
            try? FileManager.default.removeItem(at: location)
            delegate?.videoDownload(self, finished: false, errorMessage: "External cache not attached")
            session.invalidateAndCancel()
            return
        }
