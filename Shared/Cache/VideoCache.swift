...
final class VideoCache {
    ...
    static func isAvailableOffline(video: AerialVideo) -> Bool {
        ...
        if video.url.absoluteString.starts(with: "file") {
            ...
            if video.source.isCachable {
                ...
                // Readability, not just existence — same rule as the
                // non-cachable branch below: the sandboxed wallpaper
                // extension can STAT a cache file on an external volume
                // but never READ it. Filtering here keeps such entries
                // out of the playlist instead of failing in AVAssetReader
                // and looping through the playlist forever.
                if fileManager.fileExists(atPath: videoCachePath),
                   fileManager.isReadableFile(atPath: videoCachePath) {
                    do {
                ...
                // Readability, not just existence: the sandboxed wallpaper
                // extension can STAT files on external volumes but never
                // READ them (platform profile denies /Volumes). Without
                // this, pack entries on a raw /Volumes path pass the exists
                // check and then fail inside AVAssetReader. Packs inside the
                // external cache image (mount point under /Users/Shared/)
                // pass. Companion always passes.
    ...
    static func sourcePathFor(_ video: AerialVideo) -> String {
        if video.url.isFileURL {
            ...
            return video.source.folderPath + "/" + video.url.lastPathComponent
        }
    }
}
