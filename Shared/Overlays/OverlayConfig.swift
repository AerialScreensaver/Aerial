...
struct OverlayConfig: Codable, Equatable {
    ...
    /// is surfaced only in DEBUG builds and the renderer honours it only in
    /// DEBUG builds, so a stray `true` is inert in Release.
    var alwaysShowVersion: Bool
    /// Shift overlays clear of the Dock and menu bar in wallpaper mode
    /// (the screensaver never applies the offset — nothing to clear there).
    var dockOffsetEnabled: Bool
