...
struct OverlayRootView: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ZStack {
                ...
                // Version banner: independent of whether any overlays are
                // configured — `showVersionIfNeeded()` fires on every
                // saver start, and an empty layout must not gate it (the
                // banner vanished on overlay-less screens when it lived
                // inside the config branch).
    ...
    /// Bottom-right version banner, margins from the layout when one
    /// exists (`.empty` defaults otherwise).
    @ViewBuilder
