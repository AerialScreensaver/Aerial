...
class OverlayConfigManager {
    ...
    /// Drop the in-memory cache and re-read overlay-config.json. Needed
    /// by long-lived processes (the wallpaper extension) that must pick
    /// up Companion's overlay edits after their first lazy load.
    func reloadFromDisk() {
        queue.sync(flags: .barrier) {
            loadFromDisk()
            hasLoaded = true
