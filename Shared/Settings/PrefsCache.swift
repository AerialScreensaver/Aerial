...
struct PrefsCache {
    ...
    // Cache limit in GB (decimal, like the slider label and every measured
    // size — `Cache.size()` divides bytes by 1e9). Applies to the cache
    // folder only; Expansion packs are outside it. Ignored when
    // `unlimitedCache` is true.
    ...
    // Sparsebundle on an external drive backing the cache; nil when the
    // cache is a plain folder. See `Cache.isExternalImageMode`.
    static var externalCacheImagePath: String? {
        get { manager.getValue(forKeyPath: \.cache.externalCacheImagePath) }
        set { manager.setValue(newValue, forKeyPath: \.cache.externalCacheImagePath) }
    }

    // Expansion packs kept at the custom cache location (`<location>/Expansions`).
    static var expansionsAtCacheLocation: Bool {
        get { manager.getValue(forKeyPath: \.cache.expansionsAtCacheLocation) }
        set { manager.setValue(newValue, forKeyPath: \.cache.expansionsAtCacheLocation) }
    }
