...
enum SunWindowPlacement: Int {
    case daylight, centered
}
...
struct PrefsTime {
    ...
    // Where the sunrise/sunset window sits relative to the event
    static var intSunWindowPlacement: Int {
        get { manager.getValue(forKeyPath: \.time.intSunWindowPlacement) }
        set { manager.setValue(newValue, forKeyPath: \.time.intSunWindowPlacement) }
    }

    static var sunWindowPlacement: SunWindowPlacement {
        get { SunWindowPlacement(rawValue: intSunWindowPlacement) ?? .daylight }
        set(value) { intSunWindowPlacement = value.rawValue }
    }

    static var solarMode: SolarMode {
        get {
            return SolarMode(rawValue: intSolarMode) ?? .astronomical
        }
        set(value) {
    ...
    // Cached Night Shift sunrise/sunset (timeIntervalSinceReferenceDate)
    static var cachedNightShiftSunrise: Double {
        get { manager.getValue(forKeyPath: \.time.cachedNightShiftSunrise) }
        set { manager.setValue(newValue, forKeyPath: \.time.cachedNightShiftSunrise) }
    }

    static var cachedNightShiftSunset: Double {
        get { manager.getValue(forKeyPath: \.time.cachedNightShiftSunset) }
        set { manager.setValue(newValue, forKeyPath: \.time.cachedNightShiftSunset) }
    }
}
