...
final class TimeManagement: NSObject {
    ...
    /// Extract the sunrise/sunset pair from Solar using the same mode-aware
    /// cascade as getTimeSlice().
    private func solarSunriseSunset() -> (Date, Date)? {
        guard let sol = solar else { return nil }
        return sol.sunriseSunset(for: PrefsTime.solarMode)
    }
    ...
    private func computeNextTransition(sunrise: Date, sunset: Date) -> Date? {
        ...
        return SunSliceBoundaries(sunrise: nsunrise, sunset: nsunset).nextTransition(after: now)
    }
    ...
    func shouldRestrictPlaybackToDayNightVideo() -> (Bool, String) {
        ...
        if PrefsTime.timeMode == .locationService {
            ...
            guard let sunrise, let sunset else {
                errorLog("Night Shift is available but reported no sunrise/sunset times")
                return (false, "")
            }
    ...
    private func dayNightCheck(sunrise: Date, sunset: Date) -> String {
        ...
        return SunSliceBoundaries(sunrise: nsunrise, sunset: nsunset).slice(at: now)
    }
