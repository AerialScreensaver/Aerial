...
struct SourceInfo {
    ...
    static let timeInformation = [
        // macOS 27 additions (Golden Gate; the Day one keeps the default)
        "3FD9FD5C-6DC5-4362-9932-4087C942F1C5": "sunset",   // Golden Gate Evening
        "4207734D-74FE-4F92-B5E1-6EC8DEE24A15": "sunset",   // Golden Gate Sunset
        "86E89C23-C39B-44C8-A985-E56EEA6456FE": "night",    // Golden Gate Night
    ...
    //
    // `existing` must be the current rebuild's accumulator, NOT
    // VideoList.instance.videos: refreshVideoList builds into a local and
    // assigns once, so the published list is a full generation stale here —
    // scanning it either skips dedup entirely (first refresh) or flags every
    // asset as a dupe of the previous generation (subsequent refreshes).
    static func findDuplicate(id: String, url1080pH264: String, in existing: [AerialVideo]) -> (Bool, AerialVideo?) {
        ...
        for (pid, replace) in dupePairs where id == pid {
            for vid in existing where vid.id == replace {
                // Found dupe pair
                return (true, vid)
            }
        }
