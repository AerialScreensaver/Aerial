...
class VideoList {
    ...
    /// Serializes `refreshCatalogFromDisk()` — the wallpaper extension
    /// calls it from the control-listener thread AND from renderer
    /// queues (ExtensionVideoLoader's resolve self-heal).
    private let catalogRefreshLock = NSLock()
    ...
    /// Source names with a manifest download in flight. `downloadManifestsIfNeeded`
    /// runs several times during a launch (init, My Videos rescan, source
    /// toggles) and the not-cached / feed-upgrade conditions stay true
    /// until the download lands, so without this each call queued the
    /// same tar again (three installs of resources-27-0-1.tar observed
    /// in one launch). Cleared when the batch's completion runs, so a
    /// failed download is retried by the next call.
    private let manifestFetchLock = NSLock()
    private var manifestFetchesInFlight: Set<String> = []
    ...
    func reloadSources(force: Bool = false) {
        ...
        // (notably ExtensionVideoLoader) see a
    ...
    /// Read-only catalog refresh for the wallpaper extension: re-scan
    /// the Sources roots and re-parse every enabled cached entries.json.
    ///
    /// `SourceList.list` is frozen at first access, so a source created
    /// AFTER this process started — My Videos on first launch (the
    /// wizard activates the extension a minute before Companion creates
    /// it), Live Feeds, a freshly installed Expansion — never appears
    /// until a respawn; and `videos` is only rebuilt by `reloadSources()`
    /// (init / settings-changed), so a file added to My Videos
    /// mid-session is invisible too. Either way every persisted-playlist
    /// entry for that source fails to resolve and the saver silently
    /// falls back to a random rotation video (2026-08-25 field bundle:
    /// two days of that on a never-respawned extension).
    ///
    /// Unlike `reloadSources()` this never downloads a manifest and never
    /// rewrites a local source's entries.json (`updateLocalSource`), so
    /// it can't race Companion's writer — a concurrent rescan of My
    /// Videos would mint fresh ids for new files and the playlist
    /// Companion just wrote would stop matching.
    func refreshCatalogFromDisk(reason: String) {
        catalogRefreshLock.lock(); defer { catalogRefreshLock.unlock() }
        SourceList.rescan()
        refreshVideoList()
        debugLog("Catalog refreshed from disk (\(reason)): \(SourceList.list.count) sources, \(videos.count) videos")
    }
    ...
    private func downloadManifestsIfNeeded(force: Bool = false) {
        ...
        func alreadyInFlight(_ source: Source) -> Bool {
            manifestFetchLock.lock(); defer { manifestFetchLock.unlock() }
            return manifestFetchesInFlight.contains(source.name)
        }
        ...
        for source in SourceList.list {
            ...
            if source.isEnabled() {
                ...
                if alreadyInFlight(source) {
                    debugLog("\(source.name) manifest download already in flight, not re-queuing")
                } else if !source.isCached() {
                    debugLog("\(source.name) is not cached, downloading...")
                    sourceQueue.append(source)
                } else if source.needsFeedUpgrade() {
                    // The installed tar isn't the one this build points at
                    // (feed bump, or a staged install that never finished).
                    // Independent of the weekly window; still needs network.
                    let wanted = URL(string: source.manifestUrl)?.lastPathComponent ?? source.manifestUrl
                    if Cache.canNetwork() {
                        debugLog("🍎 [AppleFeed] \(source.name): installed feed ≠ \(wanted), upgrading...")
                        sourceQueue.append(source)
                    } else {
                        debugLog("🍎 [AppleFeed] \(source.name): feed upgrade to \(wanted) pending (no network)")
                    }
        ...
        // Built here, after `sourceQueue` is final, so the closure captures
        // an immutable name list (a name that ends up skipped below is
        // simply never in the set — removing it is a no-op).
        let queuedNames = sourceQueue.map { $0.name }
        let completion = BlockOperation {
            self.manifestFetchLock.lock()
            for name in queuedNames { self.manifestFetchesInFlight.remove(name) }
            self.manifestFetchLock.unlock()
            self.refreshVideoList()
        }
        ...
        if !sourceQueue.isEmpty {
            ...
            for source in sourceQueue {
                ...
                completion.addDependency(operation)
                didQueue = true
                manifestFetchLock.lock()
                manifestFetchesInFlight.insert(source.name)
                manifestFetchLock.unlock()
    ...
    private func refreshVideoList(fireCallbacks: Bool = true) {
        // Build into a local and assign once: `videos` is read concurrently
        // (DownloadCoordinator's workQueue evaluation, the extension's
        // playlist survives-check), so it must never be observably empty
        // mid-rebuild — a transient [] here used to cascade into empty
        // persisted playlists and a visible cut on paused wallpapers.
        //
        // Cross-source dedup runs against `rebuilt`, never the published
        // `videos`: that array is a full generation stale during this loop,
        // so using it either skips dedup (first refresh — every id shared
        // between manifests lands twice) or marks everything as a dupe of
        // the previous generation (later refreshes — the list collapses to
        // empty). Both states shipped once; see the log signature
        // "Playlist raw count: 0" / inflated counts.
        var rebuilt: [AerialVideo] = []

        // A refresh follows manifest downloads / catalog rescans — the
        // strings for newly published assets live in the freshly
        // extracted TVIdleScreenStrings.bundle, so re-read it.
        ...
        for source in SourceList.list {
            if source.isEnabled() {
                // We may need to download it
                if source.isCached() {
                    let vids = source.getVideos(dedupingAgainst: rebuilt)
                    rebuilt.append(contentsOf: vids)
                }
            }
        }

        videos = rebuilt.unique(for: \.id).sorted { $0.name < $1.name }
    ...
    func videosMatchingFilter(mode: NewShouldPlay, filterStrings: [String]) -> [AerialVideo] {
        switch mode {
        case .everything:
            // No suboptions — filterStrings are ignored.
            return everythingRotation()
        case .favorites:
            return videos
                .filter { PrefsVideos.favorites.contains($0.id) && !PrefsVideos.hidden.contains($0.id) }
    ...
    func pluckOrientedVideo(isVertical: Bool) -> AerialVideo? {
        // Grab the first one matching the orientation. None is a real
        // outcome (a playlist made only of portrait-flagged local videos
        // asked for landscape) — callers fall back, never trap.
        guard let match = playlist.first(where: { $0.isVertical == isVertical }) else {
            return nil
        }
        lastPluckedFromPlaylist = match

        // And actually remove it
        playlist = playlist.filter { $0 != match }
