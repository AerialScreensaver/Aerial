...
struct SourceList {
    // Apple's macOS aerials feed. The source NAME is deliberately
    // version-agnostic ("macOS", not "macOS 27"): it doubles as the
    // on-disk folder, the `sourcesEnabled` key and the `source:` filter
    // string, so a yearly rename meant a yearly user-state migration.
    // Bumping the feed is now: update `manifestUrl` + `appleMacFeedVersion`;
    // `Source.needsFeedUpgrade()` notices the new tar name and re-fetches
    // at the next launch (staged install, see FileHelpers). Folders from
    // the old naming are folded in by `seedAppleMacSourceIfNeeded()`.
    // (Those URLs are easy-ish to find; the tvOS ones less so.)
    static let appleMacSourceName = "macOS"
    static let appleMacFeedVersion = "macOS 27"
    static let macOS = Source(name: appleMacSourceName,
                        description: "High framerate videos from Apple's \(appleMacFeedVersion) feed",
                        manifestUrl: "https://sylvan.apple.com/itunes-assets/Aerials126/v4/82/2e/34/822e344c-f5d2-878c-3d56-508d5b09ed61/resources-27-0-1.tar",
                        type: .macOS,
    ...
    nonisolated(unsafe) private static var _list: [Source] = [macOS, tvOS26, tvOS13] + foundSources

    /// The source catalog. Reads hand back a snapshot (copy-on-write, so
    /// cheap); writes replace it under a lock. A `rescan()` on one queue
    /// can therefore never tear an array a renderer is iterating on
    /// another. `list.append(...)` is a read-modify-write — fine for the
    /// install flows that use it, which never run concurrently.
    static var list: [Source] {
        get { listLock.lock(); defer { listLock.unlock() }; return _list }
        set { listLock.lock(); defer { listLock.unlock() }; _list = newValue }
    }
    // static var list: [Source] = foundSources

    /// Rebuild `list` from disk. Call after the Expansions root changes,
    /// a drive mounts/unmounts, or packs are moved — `list` is otherwise
    /// frozen at first access (installs only append).
    static func rescan() {
        list = [macOS, tvOS26, tvOS13] + foundSources
    }

    /// The root a NEW source install should land in: non-cacheable
    /// (Expansion) sources go to the external root when the cache
    /// location hosts Expansions and is available (image attached);
    /// everything else uses the default root.
    /// Live Feeds is default-root managed regardless (it parses as a
    /// non-cacheable source but isn't an expansion pack).
    static func installRoot(for source: Source) -> String {
        if !source.isCachable, source.name != "Live Feeds", let external = Cache.externalSourcesRoot {
            return external
        }
        return Cache.defaultSourcesRoot
    }
    ...
    // MARK: - Apple macOS feed: legacy folder seeding / cleanup
    //
    // Builds up to 4.1.0beta17 named the Apple source after the OS
    // release ("macOS 26"), so `Sources/macOS 26/` holds that user's last
    // downloaded feed + strings bundle. The source is now "macOS". Two
    // exact-name one-shots:
    //
    // * `seedAppleMacSourceIfNeeded()` COPIES the newest legacy folder to
    //   `Sources/macOS` while that has no entries.json. Runs from
    //   `foundSources` (both processes, idempotent, ~6 MB once) so the
    //   catalog, persisted playlists and the reaper's "every enabled
    //   source has an entries.json" guard keep working offline until the
    //   new tar lands — `Source.needsFeedUpgrade()` fetches it right after.
    //   Copy, never move: loginwindow keeps the PREVIOUS appex build alive
    //   across an update and it still reads `Sources/macOS 26/`.
    // * `removeLegacyAppleMacFoldersIfNeeded()` deletes the legacy
    //   folder(s). Companion-only, and only once the running wallpaper
    //   extension is a build that knows "macOS" (WallpaperExtensionHealth).

    /// Newest first — the seed takes the first one with an entries.json.
    static let legacyAppleMacFolderNames = ["macOS 26", "macOS 15", "macOS 14"]

    static func seedAppleMacSourceIfNeeded() {
        let fm = FileManager.default
        let root = Cache.defaultSourcesRoot
        let target = root + "/" + appleMacSourceName
        guard !fm.fileExists(atPath: target + "/entries.json") else { return }
        guard let legacy = legacyAppleMacFolderNames.first(where: {
            fm.fileExists(atPath: root + "/\($0)/entries.json")
        }) else { return }

        // Copy to a sibling first, then rename into place: the rename is
        // atomic, so a second process racing us (Companion + extension
        // both touch SourceList at launch) either sees no folder or a
        // complete one, never a half-copied strings bundle.
        let staging = root + "/.\(appleMacSourceName)-seeding"
        try? fm.removeItem(atPath: staging)
        do {
            try fm.copyItem(atPath: root + "/" + legacy, toPath: staging)
            // An entries-less `macOS` folder can exist (aborted download):
            // replace it wholesale so the seed is clean.
            if fm.fileExists(atPath: target) { try fm.removeItem(atPath: target) }
            try fm.moveItem(atPath: staging, toPath: target)
            debugLog("🍎 [AppleFeed] seeded \(appleMacSourceName) from \(legacy)")
        } catch {
            try? fm.removeItem(atPath: staging)
            if fm.fileExists(atPath: target + "/entries.json") {
                debugLog("🍎 [AppleFeed] \(appleMacSourceName) was seeded concurrently — fine")
            } else {
                errorLog("🍎 [AppleFeed] couldn't seed \(appleMacSourceName) from \(legacy): \(error.localizedDescription)")
            }
        }
    }

    static func removeLegacyAppleMacFoldersIfNeeded() {
        let fm = FileManager.default
        let root = Cache.defaultSourcesRoot
        // Never drop the legacy data while the new folder isn't usable.
        guard fm.fileExists(atPath: root + "/\(appleMacSourceName)/entries.json") else { return }
        for name in legacyAppleMacFolderNames {
            let path = root + "/" + name
            guard fm.fileExists(atPath: path) else { continue }
            do {
                try fm.removeItem(atPath: path)
                debugLog("🧹 Removed legacy Apple source folder: \(name)")
            } catch {
                errorLog("Couldn't remove legacy Apple source folder \(name): \(error.localizedDescription)")
            }
        }
    }
    ...
    static var foundSources: [Source] {
        ...
        // …and make sure the Apple macOS folder exists before anything
        // asks `macOS.isCached()`.
        seedAppleMacSourceIfNeeded()

        var sources: [Source] = []

        // Look in every Sources root: the default one, plus the optional
        // external Expansions root. The default root comes first and wins
        // name collisions.
        for root in Cache.sourcesRoots {
            let isDefaultRoot = (root == Cache.defaultSourcesRoot)
            // Enumerate explicitly (loud on failure): the sandboxed
            // extension's URL(fileURLWithPath:) can't always stat
            // /Volumes paths, which made the subDirectories helper's
            // hasDirectoryPath guard silently return [] — no error, no
            // scan. isDirectory: true removes the stat dependency.
            ...
            do {
                ...
                errorLog("Couldn't scan sources root \(root): \(error.localizedDescription)")
                continue
            }
            var foundHere = 0
            for folder in folders {
                let folderName = folder.lastPathComponent
                if folderName.starts(with: "tvOS")
                    || folderName.starts(with: "macOS")
                    || folderName.starts(with: "backups") {
                    continue
                }
                // Live Feeds and My Videos are managed in the default
                // root only — a copy on the external root (e.g. from an
                // earlier accidental move) is inert.
                if !isDefaultRoot && (folderName == "Live Feeds" || folderName == "My Videos") {
                    debugLog("Skipping \(folderName) in external root (default-root managed)")
                    continue
                }

                // If it's valid, let's add !
                if let source = loadManifest(url: folder, root: root) {
                    if sources.contains(where: { $0.name == source.name }) {
                        debugLog("⚠️ Source '\(source.name)' exists in more than one root — keeping the first (\(root) copy ignored)")
                    } else {
                        sources.append(source)
                        foundHere += 1
                    }
                } else if let newsources = loadMetaManifest(url: folder, root: root) {
                    for source in newsources where !sources.contains(where: { $0.name == source.name }) {
                        sources.append(source)
                        foundHere += 1
                    }
                }
            }
            debugLog("Scanned sources root \(root): \(foundHere) source(s)")
        }
            ...
            do {
                ...
                for lurl in urls {
                    if lurl.path.lowercased().hasSuffix(".mp4") || lurl.path.lowercased().hasSuffix(".mov") {
                        ...
                        if fileSize > 100000 || fileType == .typeSymbolicLink {
    ...
    static func fetchOnlineManifest(url: URL) {
        if var source = loadManifest(url: url) {
            ...
            // New installs of non-cacheable packs land in the external
            // Expansions root when configured. Set before saveSource /
            // list.append so both the folder creation and the download
            // destination lookup resolve to the same place.
            source.rootPath = installRoot(for: source)
            saveSource(source)
    ...
    static func saveSource(_ source: Source) {
        ...
        // First make the folder (source.folderPath honors the source's
        // rootPath — callers set it via installRoot(for:) on installs)
        try? FileManager.default.createDirectory(atPath: source.folderPath, withIntermediateDirectories: true, attributes: nil)
        ...
        do {
            try json.write(to: URL(fileURLWithPath: source.folderPath + "/manifest.json"))
        } catch {
    ...
    static func saveEntries(source: Source, manifest: VideoManifest) {
        ...
        do {
            try json.write(to: URL(fileURLWithPath: source.folderPath + "/entries.json"))
        } catch {
            errorLog("Can't save local entries : \(error.localizedDescription)")
        }
    }
    static func loadMetaManifest(url: URL, root: String = "") -> [Source]? {
        ...
        do {
            ...
            if let metamanifest = try? newJSONDecoder().decode(MetaManifest.self, from: jsonData) {
                ...
                for manifest in metamanifest.sources {
                    sources.append(parseSourceFromManifest(manifest, url: url, root: root))
                }
    ...
    static func loadManifest(url: URL, root: String = "") -> Source? {
        // Let's make sure we have the required files
        if !areManifestPresent(url: url) && !url.absoluteString.starts(with: "http") {
            return nil
        }
        ...
        do {
            ...
            if let manifest = try? newJSONDecoder().decode(Manifest.self, from: jsonData) {
                return parseSourceFromManifest(manifest, url: nil, root: root)
            }
        } catch {
            errorLog("Could not open manifest for source at \(url)")
            return nil
    ...
    static private func parseSourceFromManifest(_ manifest: Manifest, url: URL?, root: String = "") -> Source {
        var local = true
        var mURL: String
        ...
        var source = Source(name: manifest.name,
                            ...
                            more: manifest.more ?? "")
        // Remember which Sources root this came from (runtime-only) —
        // empty means the default root; http installs get theirs set
        // via installRoot(for:) at save time.
        source.rootPath = (root == Cache.defaultSourcesRoot) ? "" : root
        return source
    }
    ...
    /// ["landscape"] -> [.landscape]
    ///
    /// Case-insensitive: `saveSource` writes the capitalized raw values
    /// ("City"), and only lowercase used to match — every saved pack read
    /// back as nature.
