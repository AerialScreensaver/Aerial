...
final class AerialVideo: CustomStringConvertible, Equatable {
    // Identity IS the asset id, deliberately: VideoList keeps one
    // instance per id (later manifests append to `sources` rather than
    // create duplicates), and every lookup in the app compares ids.
    // Comparing URLs too would break collection lookups that hold a
    // pre-manifest-refresh instance.
    ...
    /// Primary source — the manifest that constructed this entry, and
    /// always the head of `sources` (merges in `Source.parseVideoManifest`
    /// only ever append). Convenience for the many call sites that want
    /// "which manifest owns this entry" semantics (cache path, isCachable,
    /// sidebar grouping, inspector display).
    let source: Source
    ...
    private func getClosestAvailable(wanted: VideoFormat) -> URL {
        if let url = urls[wanted], !url.isEmpty {
            return getURL(string: url)
        }
        // Fallback order: 4K HEVC, then the macOS-manifest-only 4K SDR
        // 240, then 1080p HEVC, then 1080p H.264 as the last resort.
        for format in [VideoFormat.v4KHEVC, .v4KSDR240, .v1080pHEVC, .v1080pH264] {
            if let url = urls[format], !url.isEmpty {
                return getURL(string: url)
            }
        }
        // Something went very wrong if we are here
        errorLog("getClosestAvailable failed back hard to 4KHDR for \(id)")
    ...
    private func getURL(string: String) -> URL {
        ...
        // A malformed (or empty) manifest URL. Hand back a file URL that
        // simply fails to load rather than trapping the whole process.
        errorLog("AerialVideo \(id): malformed manifest URL '\(string)'")
        return URL(fileURLWithPath: string)
    }
    ...
    init(id: String,
        ...
        // Default stuff, we double check those below. The orientation
        // hint comes from manifests that publish it (macOS 27 dynamic
        // wallpapers, `variant.orientation`); `updateDuration()` only
        // re-probes orientation for local file:// videos, so the hint
        // sticks for remote assets and keeps portrait variants off
        // landscape screens when "favor orientation" is on.
        self.duration = 0
        self.isVertical = isVerticalHint
